...
const Login = () => {
  const{loginUserContext}=useContext(AppContext) 
  ...
  const handleSignIn = () => {
    ...
    if (!password) {
      ...
      loginUser(user)
      .then((res)=>{
        console.log(res,"reeeeeeeeeeeeeeeeeessssssss");
        setIsLoading(false)
        if (res?.status==200) {
          loginUserContext(res?.data?.data)
          Toast.show({
            type: 'success',
            text1: 'Suceess',
          ...
          console.log(res?.status,"stausssssssssssssssss");
          Toast.show({
            type: 'error',
            text1: 'Error',
            text2:res?.data?.message,
