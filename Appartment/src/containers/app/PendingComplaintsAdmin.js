...
import { getPendingComplaintsByAdmin } from '../../services/Admin';
const PendingComplaintsAdmin = () => {
  ...
  const handleGetAllComplaintsUser=()=>{
    setIsLoading(true)
    getPendingComplaintsByAdmin(admin?.userDetails?.apartmentId).then((res)=>{
      setIsLoading(false)
      if (res?.status==200) {
...
export default PendingComplaintsAdmin;
